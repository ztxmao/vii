...
func TestLPush(t *testing.T) {
	tc := New(DefaultExpiration, 0)
	err := tc.LPush("list:test", "ab")
	if err != nil {
		t.Error("Error lpush:", err)
	}
	x, found := tc.Get("list:test")
	if !found {
		t.Error("list:test not found")
	}
	fmt.Println(x)
}
...
func TestPop(t *testing.T) {
	tc := New(DefaultExpiration, 0)
	tc.LPush("pop1", "1", 2, 3, map[string]string{"sb": "abc"})
	x, found := tc.Pop("pop1")
	if !found {
		t.Error("list:test not found", x)
	}
	x, found = tc.Get("pop1")
	if found {
		t.Error("pop1 pop fail!", x)
	}
}

func TestLPop(t *testing.T) {
	tc := New(DefaultExpiration, 0)
	err := tc.LPush("list:test", "1", 2, 3, map[string]string{"sb": "abc"})
	if err != nil {
		t.Error("Error lpop:", err)
	}
	x, found := tc.LPop("list:test")
	if !found {
		t.Error("list:test not found")
	}
	if fmt.Sprintf("%v", x) == "1" {
		t.Error("list:test pop value err:", x)
	}
}
